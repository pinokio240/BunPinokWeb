...
## 2026-08-13 — v0.9.1 — Диспетчер задач + настройки-хаб

### Диспетчер задач (browser://tasks)
- [x] Таблица процессов (app.getAppMetrics): задача/память/ЦП/PID, метки (вкладка/расширение/системный)
- [x] «Завершить» — forcefullyCrashRenderer() для вкладок/расширений
- [x] Обновление каждые 2с, итоги (процессов/памяти)
- [x] Вход: Shift+Esc, ⋮-меню, меню «Вид», сайдбар настроек

### Настройки как хаб
- [x] Все страницы меню теперь в сайдбаре настроек: Загрузки, История, Закладки, Пароли, Расширения, Приватность, Журнал браузера, Диспетчер задач, О браузере

### VK Music Saver
- [x] Фикс установки: ключ `declarative_net_request` без разрешения отклонялся Electron → ключ удаляется, статические DNR-правила (Genius UA/CORS) вшиваются в шим и регистрируются в мосте
- [x] Баннер CWS: повторные попытки (SPA-рендер), перехват родной кнопки «Добавить в Chrome», логи `[bunpinok-install]`
...
- SW после фикса wrapper'а жил (commands-register шёл), НО нативный `chrome.runtime.sendMessage` из попапа в MV3-SW падал: `NOTREACHED hit. Unexpected view type found: 0` (runtime_api.cc) → попап не доставал до фона → секции не грузились
- В MV2 (фон-страница) мессенджинг работает нативно — как в сессии 14:30

### Fix
- [x] Приоритет загрузки развёрнут: `prepareExtensionForElectron(extPath, 'mv2')` СНАЧАЛА, нативный MV3 — только если MV2 упал
- [x] MV2-конверсия умеет извлекать оригинальный worker из `bunpinok-sw-wrapper.js` (если расширение ранее патилось в native-режиме)
- [x] Тулбар: ПКМ по иконке расширения → нативное меню: «Открыть попап» (если есть), «Настройки» (если объявлены в манифесте), «Управление расширениями»
- [x] Попап: `exposeInMainWorld` guard сработал — в свежих сессиях ошибка инжекта ушла
